    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (helpers_1.Helpers.isFolderEmpty(repoDir)) {
                ...
                yield gitService.clone(remoteUrl, fetchDepth);
            }
            core.debug(`🤖 Intializing git config at: '${repoDir}'`);
            yield gitService.init(remoteUrl, authorName, authorEmail);
            ...
            if (gitService.hasChanges()) {
                ...
                yield gitService.cleanCheckoutBranch(prBranch, baseBranch, remotePrBranchExists);
            ...
            if (deleteClosedPRBranches) {
                core.info(`🤖 Checking for closed PRs created by '${authorName}' on '${baseBranch}'...`);
                const closedPRsBranches = yield gbService.getClosedPRsBranches(baseBranch, authorName);
                if (closedPRsBranches.length) {
                    core.info(`🤖 Deleting branches related to closed PRs created by '${authorName}': ${closedPRsBranches} ...`);
                    yield gitService.raw(['push', 'origin', '--delete', ...closedPRsBranches]);
                }
            }
